...
async fn run_client(name: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    tracing::info!("Response={:?}", response.into_inner().message);
    Ok(())
}
...
impl Greeter for MyGreeter {
    async fn say_hello(&self, request: Request<HelloRequest>) -> Result<Response<HelloResponse>, Status> {
        let reply = HelloResponse { message: format!("Hello {}!", request.into_inner().name) };
...
async fn test_grpc_req_resp() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Greeting Server listening on {}", address);

    tokio::spawn(Server::builder().add_service(GreeterServer::new(greeter)).serve(address));
